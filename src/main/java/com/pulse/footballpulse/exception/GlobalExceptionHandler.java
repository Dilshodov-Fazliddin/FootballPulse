...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> dataNotFoundExceptionHandler(
        ...
        return ResponseEntity.status(404).body(ApiResponse.builder().status(404).message(e.getMessage()).data(null).build());
    }
    ...
    public ResponseEntity<ApiResponse<Object>> notAcceptableException(NotAcceptableException e) {
        return ResponseEntity.status(409).body(ApiResponse.builder().status(409).message(e.getMessage()).data(null).build());
    }

    @ExceptionHandler(value = {ForbiddenException.class})
    @ResponseStatus(value = HttpStatus.FORBIDDEN)
    public ResponseEntity<ApiResponse<?>> forbiddenException(ForbiddenException e) {
        return ResponseEntity.status(403).body(ApiResponse.builder().status(409).message(e.getMessage()).data(null).build());
    }
}
